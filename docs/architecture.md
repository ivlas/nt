...
Queryless context computes a canonical frontier over history before the bounded
recent raw tail. The frontier prefers the largest completed summary at each
...
separators, and newlines consume the same budget as memory content. Storage size
and context size are independent: the append-only history may grow without
changing the bounded context allocation. `nt memory expand` reveals exactly one
